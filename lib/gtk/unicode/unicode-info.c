...
const gchar *
font_manager_unicode_get_codepoint_data_name (gunichar uc)
{
    ...
    while (max >= min) {
        ...
        if (uc > unicode_names[mid].index)
            ...
            return font_manager_unicode_name_get_name(&unicode_names[mid]);
    }
 ...
 * @ch: a #gunichar
 *
 * Returns: (transfer none) (nullable): unicode character name or %NULL
 */
const gchar *
font_manager_unicode_get_codepoint_name (gunichar ch)
{
    static gchar buf[32];
    ...
    if G_UNLIKELY(is_cjk_unified(ch)) {
        ...
        const gchar *x = font_manager_unicode_get_codepoint_data_name(ch);
        if (x == NULL)
 ...
 * @ch: a #gunichar
 *
 * Returns: (transfer none) (nullable): unicode category name or %NULL
 */
const gchar *
font_manager_unicode_get_category_name (gunichar ch)
{
...
gint
font_manager_unicode_get_codepoint_data_name_count (void)
{
...
font_manager_unicode_get_version (gunichar uc)
{
    ...
    if (uc < unicode_versions[0].start || uc > unicode_versions[max].end)
        return FONT_MANAGER_UNICODE_VERSION_UNASSIGNED;
...
const gchar *
font_manager_unicode_version_to_string (FontManagerUnicodeStandard version)
{
    g_return_val_if_fail(version >= FONT_MANAGER_UNICODE_VERSION_UNASSIGNED, NULL);
    g_return_val_if_fail(version <= FONT_MANAGER_UNICODE_VERSION_LATEST, NULL);

    if G_UNLIKELY(version == FONT_MANAGER_UNICODE_VERSION_UNASSIGNED)
        return NULL;
...
gint
font_manager_unicode_get_unihan_count (void)
{
...
static const FontManagerNamesList *
get_nameslist (gunichar uc)
{
  static gunichar most_recent_searched;
  static const FontManagerNamesList *most_recent_result;
  gint min = 0;
  gint mid;
...
gunichar *
font_manager_unicode_get_nameslist_exes (gunichar uc)
{
  const FontManagerNamesList *nl;
  gunichar *exes;
  gunichar i, count;
...
/**
 * font_manager_unicode_get_nameslist_equals:
 * @uc: a gunichar
 *
 ...
 */
const gchar **
font_manager_unicode_get_nameslist_equals (gunichar uc)
{
  const FontManagerNamesList *nl;
  const gchar **equals;
  gunichar i, count;
...
/**
 * font_manager_unicode_get_nameslist_stars:
 * @uc: a #gunichar
 *
 * Returns: (transfer container) (array zero-terminated=1) (element-type utf8):
 * newly allocated %NULL-terminated array of gchar*, the items are const,
 ...
 */
const gchar **
font_manager_unicode_get_nameslist_stars (gunichar uc)
{
  const FontManagerNamesList *nl;
  const gchar **stars;
  gunichar i, count;
...
/**
 * font_manager_unicode_get_nameslist_pounds:
 * @uc: a #gunichar
 *
 * Returns: (transfer container) (array zero-terminated=1) (element-type utf8):
 * newly allocated %NULL-terminated array of gchar*, the items are const,
 ...
 */
const gchar **
font_manager_unicode_get_nameslist_pounds (gunichar uc)
{
  const FontManagerNamesList *nl;
  const gchar **pounds;
  gunichar i, count;
...
/**
 * font_manager_unicode_get_nameslist_colons:
 * @uc: a #gunichar
 *
 * Returns: (transfer container) (array zero-terminated=1) (element-type utf8):
 * newly allocated %NULL-terminated array of gchar*, the items are const,
 ...
 */
const gchar **
font_manager_unicode_get_nameslist_colons (gunichar uc)
{
  const FontManagerNamesList *nl;
  const gchar **colons;
  gunichar i, count;
...
gboolean
font_manager_unicode_unichar_validate (gunichar ch)
{
  return g_unichar_validate (ch);
}

/**
 * font_manager_unicode_unichar_to_printable_utf8:
 * @uc: a unicode character
 * @outbuf: (out caller-allocates): output buffer, must have at least 10 bytes of space.
 * If %NULL, the length will be computed and returned, nothing will be written to @outbuf.
 *
...
/**
 * font_manager_unicode_unichar_isgraph:
 * @uc: a Unicode character
 *
 ...
 **/
gboolean
font_manager_unicode_unichar_isgraph (gunichar uc)
{
