...
G_DEFINE_TYPE(UnicodeSearchBar, unicode_search_bar, GTK_TYPE_WIDGET)
                                      ...
                                      UnicodeCharacterMap *character_map)
{
    ...
    if (self->charmap) {
        g_signal_connect_after(self->charmap, "notify::font-desc", G_CALLBACK(reset_search), self);
        g_signal_connect_after(self->charmap, "notify::codepoint-list", G_CALLBACK(reset_search), self);
    }
    return;
}
