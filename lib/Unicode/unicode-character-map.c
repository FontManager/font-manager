                                              ...
                                              PangoFontDescription *font_desc /* adopting */)
{
    ...
    if (gtk_widget_get_realized (GTK_WIDGET(charmap)))
                ...
                gint col)
{
    ...
    wc = unicode_codepoint_list_get_char(priv->codepoint_list, cell);

    if (wc > UNICHAR_MAX || !unicode_unichar_validate(wc) || !unicode_unichar_isdefined(wc))
        return;

    n = unicode_unichar_to_printable_utf8(wc, buf);
    pango_layout_set_text(priv->pango_layout, buf, n);

    /* XXX: Keep the square empty if the font has no glyph for this cell. */
                ...
                gint col)
{
    ...
    if (gtk_widget_has_focus(widget) && cell == priv->active_cell)
        ...
        _state = GTK_STATE_FLAG_INSENSITIVE | GTK_STATE_FLAG_ACTIVE;
    else if (!unicode_unichar_validate(wc) || !unicode_unichar_isdefined(wc))
...
static void
set_active_char (UnicodeCharacterMap *charmap, gunichar wc)
{
...
vadjustment_value_changed_cb (GtkAdjustment *vadjustment, UnicodeCharacterMap *charmap)
{
    ...
    unicode_character_map_set_active_cell(charmap, priv->page_first_cell + r * priv->cols + c);
    return;
}
...
static void
unicode_character_map_drag_data_get (GtkWidget *widget,
...
{
    gchar buf[7];
    gint n = g_unichar_to_utf8(unicode_codepoint_list_get_char(priv->codepoint_list, priv->active_cell), buf);
...
unicode_character_map_style_updated (GtkWidget *widget)
{
    ...
    if (priv->font_desc == NULL) {
        GtkStyleContext *ctx = gtk_widget_get_style_context(widget);
        const PangoFontDescription *_font_desc;
...
static void
unicode_character_map_paste_received_cb (GtkClipboard *clipboard,
                                         const char *text,
                                         gpointer user_data)
{
...
unicode_character_map_init (UnicodeCharacterMap *charmap)
{
    ...
    unicode_character_map_set_codepoint_list(charmap, NULL);
...
unicode_character_map_class_init (UnicodeCharacterMapClass *klass)
{
    ...
    g_object_class_install_property(object_class,
                                    ...
                                    g_param_spec_object("codepoint-list",
                                                        NULL, NULL,
                                                        unicode_codepoint_list_get_type (),
                                                        G_PARAM_READWRITE |
                                                        G_PARAM_STATIC_NAME |
                                                        G_PARAM_STATIC_NICK |
                                                        G_PARAM_STATIC_BLURB));
 ...
 *
 * Returns: a new #UnicodeCharacterMap
 */
GtkWidget *
 ...
 * @charmap: a #UnicodeCharacterMap
 * @codepoint_list: a #UnicodeCodepointList
 *
                                          ...
                                          UnicodeCodepointList *codepoint_list)
{
    ...
    if (codepoint_list)
        priv->last_cell = unicode_codepoint_list_get_last_index(codepoint_list);
    else
        priv->last_cell = 0;
