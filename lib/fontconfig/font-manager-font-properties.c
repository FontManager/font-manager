...
#include "font-manager-font-properties.h"

/**
 * SECTION: font-manager-font-properties
...
typedef struct
{
    ...
    FontManagerFontPropertiesType type;
}
...
GType
font_manager_font_properties_type_get_type (void)
{
  ...
  if (g_once_init_enter (&g_define_type_id__volatile))
    {
      static const GEnumValue values[] = {
        { FONT_MANAGER_FONT_PROPERTIES_TYPE_DEFAULT, "FONT_MANAGER_FONT_PROPERTIES_TYPE_DEFAULT", "default" },
        { FONT_MANAGER_FONT_PROPERTIES_TYPE_DISPLAY, "FONT_MANAGER_FONT_PROPERTIES_TYPE_DISPLAY", "display" },
        { 0, NULL, NULL }
      };
      GType g_define_type_id =
        g_enum_register_static (g_intern_static_string ("FontManagerFontPropertiesType"), values);
...
static void
font_manager_font_properties_dispose (GObject *gobject)
{
                                     ...
                                     GParamSpec *pspec)
{
    ...
    switch (property_id) {
        ...
        case PROP_TARGET_FILE:
            ...
            break;
        case PROP_TYPE:
            priv->type = ((FontManagerFontPropertiesType) g_value_get_int(value));
            break;
        default:
            G_OBJECT_WARN_INVALID_PROPERTY_ID(gobject, property_id, pspec);
            break;
...
font_manager_font_properties_parse_match_node (FontManagerFontProperties *self, xmlNode *match_node)
{
    for (xmlNode *iter = match_node->children; iter != NULL; iter = iter->next) {
        if (iter->type != XML_ELEMENT_NODE) {
            ...
            FONT_MANAGER_FONT_PROPERTIES_GET_CLASS(self)->parse_edit_node(self, iter);
        } else if (g_strcmp0((const char *) iter->name, "test") == 0) {
            FONT_MANAGER_FONT_PROPERTIES_GET_CLASS(self)->parse_test_node(self, iter);
        }
    }
    return;
}
...
font_manager_font_properties_class_init (FontManagerFontPropertiesClass *klass)
{
    ...
    object_class->dispose = font_manager_font_properties_dispose;
    object_class->get_property = font_manager_font_properties_get_property;
    object_class->set_property = font_manager_font_properties_set_property;

    klass->parse_edit_node = font_manager_font_properties_parse_edit_node;
    klass->parse_test_node = font_manager_font_properties_parse_test_node;
    klass->add_match_criteria = font_manager_font_properties_add_match_criteria;
...
/**
 * font_manager_font_properties_load:
 * @self:   #FontManagerFontProperties
 *
 * Returns: %TRUE if current configuration file was successfully loaded
 */
gboolean
font_manager_font_properties_load (FontManagerFontProperties *self)
{
    ...
    for (xmlNode *iter = root->children; iter != NULL; iter = iter->next) {
        ...
        if (g_strcmp0((const char *) iter->name, "match") == 0) {
            font_manager_font_properties_parse_match_node(self, iter);
            break;
        }
    }
...
/**
 * font_manager_font_properties_save:
 * @self:   #FontManagerFontProperties
 *
 * Returns: %TRUE if current configuration was successfully saved to file
 */
gboolean
font_manager_font_properties_save (FontManagerFontProperties *self)
{
    g_return_val_if_fail(self != NULL, FALSE);
    g_autofree gchar *filepath = font_manager_font_properties_get_filepath(self);
    ...
    FONT_MANAGER_FONT_PROPERTIES_GET_CLASS(self)->add_match_criteria(self, writer);
    font_manager_font_properties_add_assignments(self, writer);
...
/**
 * font_manager_font_properties_discard:
 * @self:   #FontManagerFontProperties
 *
 * Returns: %TRUE if current configuration was successfully discarded
 */
gboolean
font_manager_font_properties_discard (FontManagerFontProperties *self)
{
    g_return_val_if_fail(self != NULL, FALSE);
    g_autofree gchar *filepath = font_manager_font_properties_get_filepath(self);
    g_autoptr(GFile) file = g_file_new_for_path(filepath);
...
/**
 * font_manager_font_properties_get_filepath:
 * @self:   #FontManagerFontProperties
 *
...
/**
 * font_manager_font_properties_reset:
 * @self:   #FontManagerFontProperties
 *
...
/**
 * font_manager_font_properties_new:
 *
 * Returns: (transfer full): A newly created #FontManagerFontProperties
 * Free the returned object using #g_object_unref().
 */
FontManagerFontProperties *
font_manager_font_properties_new (void)
{
