...
{
    TestDialog *dialog = test_dialog_new(parent, "Collection List", 400, 550);
    ...
    g_signal_connect(collections, "selection-changed", G_CALLBACK(on_selection_changed), NULL);
    g_list_free_full(families, g_free);
    test_dialog_append(dialog, collections);
    return dialog;
}
